...
DEPLOY_VERSION = "2026-03-13-v5"  # Bump on each deploy to verify code freshness
...
async def analyze(file: UploadFile = File(...)):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Sanitize hash for filesystem use: replace colon with dash (Windows doesn't allow colons in dir names)
        hash_for_path = analysis_hash.replace(":", "-")
        out_dir = os.path.abspath(os.path.join(STORAGE_ROOT, hash_for_path))
